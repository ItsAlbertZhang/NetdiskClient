...
static int flushmode_enable(struct termios *tm_old) {
    int ret = 0;
    ...
    // 保存现在的终端类型(缓冲型)
    ret = tcgetattr(fd, &tm);
    RET_CHECK_BLACKLIST(-1, ret, "tcgetattr");
    *tm_old = tm;
    // 更改终端设置为原始的非缓冲型，该模式下所有的输入数据以字节为单位被处理
    cfmakeraw(&tm);
    ret = tcsetattr(fd, TCSANOW, &tm);
    RET_CHECK_BLACKLIST(-1, ret, "tcsetattr");
    // 注意: 更改终端为非缓冲型之后, \n 需要替换为 \r\n.
    // 因此, 本 .c 文件中以及 lib/lib_log.c 文件中, 换行均为 \r\n.

    // 更改 stdin 为非阻塞
    int stdin_cntl = fcntl(STDIN_FILENO, F_GETFL);
    stdin_cntl |= O_NONBLOCK;
    ret = fcntl(STDIN_FILENO, F_SETFL, stdin_cntl);
    RET_CHECK_BLACKLIST(-1, ret, "fcntl");

    return 0;
}

static int flushmode_disable(struct termios tm_old) {
    int ret = 0;
    ...
    // 恢复终端为缓冲型
    int fd = 0;
    ret = tcsetattr(fd, TCSANOW, &tm_old);
    RET_CHECK_BLACKLIST(-1, ret, "tcsetattr");
...
static int print_progress_bar(void) {
    // 访问进度条
    ...
    int ret = tasknum;
    // 准备必要资源
    int percent0o1cnt, percent5cnt, instant_speed_num, instant_speed_level, avg_speed_num, avg_speed_level;
    ...
    for (int i = 0; i < tasknum; i++) {
        ...
        // 打印输出
        printf("\r[%-20.*s] %3d.%1d%%   当前:%4d %4s  平均:%4d %4s   %s\r\n", percent5cnt, progress_output, percent0o1cnt / 10, percent0o1cnt % 10, instant_speed_num, speedps_str[instant_speed_level], avg_speed_num, speedps_str[avg_speed_level], progress_bar[index]->filename);
    }
