...
static int msg_cs_mkdir_send(int connect_fd, const struct msg_cs_mkdir_sendbuf_t *sendbuf) {
    int ret = 0;
...
static int msg_cs_mkdir_recv(int connect_fd, struct msg_cs_mkdir_recvbuf_t *recvbuf) {
    ...
    bzero(recvbuf, sizeof(struct msg_cs_mkdir_recvbuf_t));
...
int msgsend_cs_mkdir(char *cmd) {
    ...
    // 发送消息
    ret = msg_cs_mkdir_send(program_stat->connect_fd, &sendbuf);
...
int msgrecv_cs_mkdir(void) {
    ...
    // 获取服务端的回复信息
    ret = msg_cs_mkdir_recv(program_stat->connect_fd, &recvbuf);
    RET_CHECK_BLACKLIST(-1, ret, "msg_cs_mkdir_recv");

    if (DISAPPROVE == recvbuf.approve) {
        logging(LOG_ERROR, "有同名目录存在, 命令未执行.");
    }
